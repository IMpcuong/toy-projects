...
void solve()
{
  auto n = nxt<int>();
  vector<ll> reds(n);
  ranges::generate(reds, nxt<ll>);
  vector<ll> blues(n);
  ranges::generate(blues, nxt<ll>);

  ll min_score = 0, max_score = 0;
  for (int i = 0; i < n; i++)
  {
    ll cur_min = min(min_score - reds[i], blues[i] - max_score);
    ll cur_max = max(max_score - reds[i], blues[i] - min_score);
    min_score = cur_min;
    max_score = cur_max;
  }

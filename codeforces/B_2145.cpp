...
void solve()
{
  ...
  int rm_top = ranges::count(ops, '0');
  int rm_bot = ranges::count(ops, '1');
  int rm_or  = ranges::count(ops, '2');

  string ans(n, '+');
  for (int i = 0; i < n; i++)
  {
    if (i < rm_top || i >= n - rm_bot || op_cnt == n)
      ans[i] = '-';
    else if (i < rm_top + rm_or || i >= n - (rm_bot + rm_or))
      ans[i] = '?';
  }

...
void solve()
{
  auto n = nxt<int>();
  vector<int> avail(n);
  ranges::generate(avail, nxt<int>);
  vector<int> req(n + 1);
  ranges::generate(req, nxt<int>);

  ranges::sort(avail, [](const int &f, const int &s) { return f > s; });
  ranges::sort(req, [](const int &f, const int &s) { return f > s; });

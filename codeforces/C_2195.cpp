...
{
  auto n = nxt<int>();
  vector<int> roll_seq(n);
  ranges::generate(roll_seq, nxt<int>);

  vector<ar<int, FACES>> dp(n);
  for (int dice = 0; dice < FACES; dice++)
  {
    int first_roll = roll_seq.front() - 1;
    if (dice != first_roll)
      dp[0][dice] = 1;
  }
  for (int i = 1; i < n; i++)
  {
    ...
    for (int dice = 0; dice < FACES; dice++)
    {
      ...
      for (int _dice = 0; _dice < FACES; _dice++)
      {
        ...
        {
          dp[i][cur_roll] = min(dp[i][cur_roll], dp[i - 1][_dice]);
        }
      }
    }
  }

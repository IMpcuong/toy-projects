...
void solve()
{
  ...
  {
    int cur = s[i - 1] - 'a';
    for (int last = 0; last < _alphabet + 1; last++)
    {
      if (dp[i - 1][last] == INF)
        continue;

      // Delete `cur`:
      dp[i][last] = min(dp[i][last], dp[i - 1][last] + 1);

      // Keep `cur`:
      if (last == _alphabet)
      {
        dp[i][cur] = min(dp[i][cur], dp[i - 1][_alphabet]);
        continue;
      }
      if (last == cur)
        // Be able to merge:
        dp[i][_alphabet] = min(dp[i][_alphabet], dp[i - 1][last]);
      if (last != cur)
        // Removes `last` so `cur` can stay on top of the wait-list:
        dp[i][cur] = min(dp[i][cur], dp[i - 1][last] + 1);
    }
  }

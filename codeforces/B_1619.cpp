...
{
  auto upper = nxt<ll>();
  ll sqrt_val = static_cast<ll>(ceil(sqrt(upper)));
  ll cbrt_val = static_cast<ll>(ceil(cbrt(upper)));

  set<ll> ans;
  for (int num = 1; num <= sqrt_val; num++)
  {
    ll sqr_num = num * num;
    if (sqr_num <= upper)
      ans.emplace(sqr_num);
  }
  for (int num = 1; num <= cbrt_val; num++)
  {
    ll cb_num = num * num * num;
    if (cb_num <= upper)
      ans.emplace(cb_num);
  }

...
void solve()
{
  auto year = nxt<string>();
  int year_as_num = 0;
  for (int i = 0; i < sza(year); i++)
    year_as_num += pow(10, sza(year) - i - 1) * (year[i] - '0');

  int sqrt_val = static_cast<int>(sqrt(year_as_num));
  if (sqrt_val * sqrt_val != year_as_num)
  {
    println(-1);
    return;
  }
